...
interface EditSectionTeamMembersModalProps extends EditSection {
  section: TeamMembersSection
}

type EditMemberProps = {
  data?: TeamMembersSectionMember
  onSave: (data: Partial<TeamMembersSectionMember>) => void
}
      ...
      <div className="flex items-center">
        ...
        <div className="flex items-center gap-4">
          <div className="aspect-1 h-10">
            <img className="object-cover object-center bg-black w-full h-full" src={data.image} alt={data.name} />
          </div>
          <p>{data.name}</p>
        </div>
      </div>
...
function EditMember({ data, onSave }: EditMemberProps) {
    ...
    <div className="px-4">
      <div className="mb-6">
        <button type="button" className="w-full" onClick={() => setShowImageModal(true)}>
          ...
          >
            {url && <img src={url} className="object-cover object-center bg-black w-full h-full" alt={name} />}
            {!url && <ImageIcon />}
          </div>
        </button>
      </div>
...
export default function EditSectionTeamMembersModal({
  section: defaultValue,
  open,
  onClose: onBack,
  onUpdate,
}: EditSectionTeamMembersModalProps) {
  const websiteContext = useContext(WebsiteContext)

  const [section, setSection] = useState(defaultValue)
  const [sectionVariants, setSectionVariants] = useState<TeamMembersVariants[]>([])

  const [showAddMemberModal, setShowAddMemberModal] = useState(false)
  const [editMember, setEditMember] = useState<TeamMembersSectionMember>()
  ...
  const updateVariableData = (name: TeamMembersVariables, category: SectionVariableCategories, data: any) => {
  ...
  const handleVariantChange = (variant: TeamMembersVariants) => {
    setSection(_sec => ({ ..._sec, variant: variant as any }))
  }

  const handleDelete = (id: string) => {
    const newMembers = members?.data.filter((member: TeamMembersSectionMember) => member.id !== id)
    setSection({
      ...section,
      variables: section.variables.map(variable => {
        if (variable.name === 'TEAM_MEMBERS') {
  ...
  const handleEditAdd = (data: Partial<TeamMembersSectionMember>) => {
    if (data.id) {
      const newMembers = members?.data.map((member: TeamMembersSectionMember) => {
    ...
    <div>
      <ModalDialogV2 {...{ open, onBack }}>
        ...
        <div className="px-4 mt-12 space-y-5 pb-24">
          <h2 className="uppercase font-medium text-sm">Members</h2>
          <div className="space-y-5">
            {members?.data.map((member: TeamMembersSectionMember) => (
              <Item
                key={member.id}
                data={member}
                onDeleteClick={() => handleDelete(member.id)}
                onEditClick={() => setEditMember(member)}
              />
            ))}
          </div>
          <button
      ...
      >
        <EditMember data={editMember} onSave={handleEditAdd} />
      </ModalDialogV2>
    </div>
  )
}
