...
function EditItem({ data, onSave }: EdiItemProps) {
  ...
  return (
    <div className="px-4">
      <div className="mb-6">
        <button type="button" className="w-full relative" onClick={() => setShowImageModal(true)}>
          <div
            className={clsx(
              'w-full bg-white',
          ...
          >
            {url && <EditImageOverlay src={url} alt={name} />}
            {!url && <ImageIcon />}
          </div>
        </button>
      </div>
...
export default function EditSectionTextModal({ open, section: defaultValue, onClose: onBack, onUpdate }: EditSection) {
  ...
  const [sectionVariants, setSectionVariants] = useState<TextSectionVariants[]>([])
  ...
  const handleVariantChange = (variant: TextSectionVariants) => {
        ...
        <div className="px-4 space-y-8 mt-4 pb-24">
          ...
          {section.variant === 'VARIANT_11' && (
            <div className="space-y-5 pb-24">
              ...
              <button
                className="border shadow-md rounded-md hover:bg-green-600 bg-green-500 h-[65px] flex justify-center items-center text-lg px-6 w-full text-white"
                type="button"
